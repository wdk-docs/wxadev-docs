...
表单。将组件内的用户输入的 <:doc:`switch`> <:doc:`input`> <:doc:`checkbox`> <:doc:`slider`> <:doc:`radio`> <:doc:`picker`> 提交。

当点击 <:doc:`form`> 表单中 form-type 为 submit 的 <:doc:`button`> 组件时，会将表单组件中的 value 值进行提交，需要在表单组件中加上 name 来作为 key。
