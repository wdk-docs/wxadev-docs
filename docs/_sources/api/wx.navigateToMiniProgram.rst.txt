...
.. function:: wx.navigateToMiniProgram(Object object)
   ...
   :param string path: 打开的页面路径，如果为空则打开首页。path 中 ? 后面的部分会成为 query，在小程序的 :func:`App.onLaunch` 、 :func:`App.onShow` 和 :func:`Page.onLoad` 的回调函数或小游戏的 :func:`wx.onShow` 回调函数、 :func:`wx.getLaunchOptionsSync` 中可以获取到 query 数据。对于小游戏，可以只传入 query 部分，来实现传参效果，如：传入  ``"?foo=bar"`` 。
   :param object extraData: 需要传递给目标小程序的数据，目标小程序可在 :func:`App.onLaunch` ， :func:`App.onShow` 中获取到这份数据。如果跳转的是小游戏，可以在 :func:`wx.onShow` 、 :func:`wx.getLaunchOptionsSync` 中可以获取到这份数据数据。
