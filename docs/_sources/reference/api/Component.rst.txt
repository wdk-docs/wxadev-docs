:wxref:`创建自定义组件(Component) <api/Component>`
=====================================================
...
.. class:: Component(Object object)
  ...
  :param object properties:	Object Map	否	组件的对外属性，是属性名到属性设置的映射表
  :param object data: 	组件的内部数据，和 properties 一同用于组件的模板渲染
  :param object observers: 	组件数据字段监听器，用于监听 properties 和 data 的变化，参见 数据监听器	2.6.1
  :param object methods: 	组件的方法，包括事件响应函数和任意的自定义方法，关于事件响应函数的使用，参见 组件间通信与事件
  :param string behaviors:	 Array	否	类似于mixins和traits的组件间代码复用机制，参见 behaviors
  :param function created: 组件生命周期函数-在组件实例刚刚被创建时执行，注意此时不能调用 setData )
  :param function attached: 组件生命周期函数-在组件实例进入页面节点树时执行)
  :param function ready: 组件生命周期函数-在组件布局完成后执行)
  :param function moved: 组件生命周期函数-在组件实例被移动到节点树另一个位置时执行)
  :param function detached: 组件生命周期函数-在组件实例被从页面节点树移除时执行)
  :param object relations: 	组件间关系定义，参见 组件间关系
  :param string externalClasses: Array	否	组件接受的外部样式类，参见 外部样式类
  :param object options:	Object Map	否	一些选项（文档中介绍相关特性时会涉及具体的选项设置，这里暂不列举）
  :param object lifetimes: 	组件生命周期声明对象，参见 组件生命周期	2.2.3
  :param object pageLifetimes: 	组件所在页面的生命周期声明对象，参见 组件生命周期	2.2.3
  :param function definitionFilter: 定义段过滤器，用于自定义组件扩展，参见 自定义组件扩展	2.2.3
